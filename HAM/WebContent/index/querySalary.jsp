...
<body class="layui-fluid">
	...
	<c:forEach items="${requestScope.pageBean.beanList}" var="salary">
		...
		<center>
			...
			<a href="${pageContext.request.contextPath}/ListStaffSalaryServlet?currentPage=1"> <button class="layui-btn">首页</button></a>
			<c:if test="${pageBean.currentPage>1}">
				<a href="${pageContext.request.contextPath}/ListStaffSalaryServlet?currentPage=${pageBean.currentPage-1}"> <button class="layui-btn">上一页</button></a>
			</c:if>
			<c:if test="${pageBean.currentPage<pageBean.totalPages}">
				<a href="${pageContext.request.contextPath}/ListStaffSalaryServlet?currentPage=${pageBean.currentPage+1}"> <button class="layui-btn">下一页</button></a>
			</c:if>
			<a href="${pageContext.request.contextPath}/ListStaffSalaryServlet?currentPage=${pageBean.totalPages}"> <button class="layui-btn">尾页</button></a>		
		</center>
</body>
</html>